...
## v0.0.1
- 初始版本，包含基础功能模块的实现.
- 实现了主Agent和7个子Agent的委派功能.
- 实现了RAG功能，用于从Qdrant数据库中检索相关信息.
- 实现了敏感工具拦截功能，用于在工作流执行涉及敏感操作前暂停进程.
- 实现了可观测性功能，用于监控各智能体间的交互轨迹.
- 实现了模块化设计，将代码拆分为`vectorizer`和`customer_support_chat`两个独立模块.
- 实现了环境变量管理功能，用于管理项目依赖项和配置参数.
- 实现了日志记录功能，用于记录系统运行时的事件和错误信息.
- 实现了 Web 聊天界面功能，提供 HTML 形式的用户聊天界面.

## v0.0.2
- 实现了多用户支持功能，允许同时运行多个实例.
- 实现了聊天记录保存功能，用于持久化存储用户的对话历史.
- 完善了部分功能：
  - 查询分类
  - 检索策略选择
  - BM25关键词检索
  - Query改写
- 对接真实数据源，添加基于聚合数据，滴滴API，携程API，OneBound API的实时数据查询功能
    - 聚班数据查询
    - 酒店数据查询
    - 车辆数据查询
    - 其他相关数据查询
- 修复了部分bug，如聊天记录保存功能的实现问题，以及 Web 聊天界面的显示问题.

## v0.0.3
- 实现了用户认证系统，包含登录和注册页面.
  - 登录页面：支持用户名/密码登录，玻璃拟态 UI 风格与整体设计统一.
  - 注册页面：支持新用户注册（用户名、密码、确认密码），含完整表单验证.
  - 密码安全：采用 PBKDF2-SHA256（10万次迭代）+ 随机盐哈希存储，常数时间比较防时序攻击.
  - 会话管理：基于 Starlette SessionMiddleware 实现服务端签名 Cookie 会话（itsdangerous），24小时过期.
  - 认证保护：所有聊天及 API 端点均需登录认证，未登录自动重定向至登录页.
